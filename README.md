...
- **跨会话持久记忆**：对标 Claude Code 的 CLAUDE.md 持久记忆 + wU2 结构化分区，按 `agentId + adminId` 会话族隔离，自动沉淀「用户画像 / 长期约束 / 已确认事实 / 交互偏好」四类长期事实。每轮异步增量抽取，跨会话「越用越懂你」，且评估探针数据绝不污染正式记忆。
...
- **异步增量抽取**：每轮回答后由 `@Async` 线程触发，复用 `rewriteModel` 小快模型降本；四重闸门（全局开关 → 智能体开关 → 起步阈值 8 轮 → 间隔节流每 6 轮）把 LLM 调用压到极低频。旧记忆增量合并去重，新事实冲突以本次为准。
