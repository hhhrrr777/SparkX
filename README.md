...
- **跨会话持久记忆**：对标 Claude Code 的 CLAUDE.md 持久记忆 + wU2 结构化分区，按 `agentId + adminId` 会话族隔离，自动沉淀「用户画像 / 长期约束 / 已确认事实 / 交互偏好」四类长期事实。达到起步阈值（默认 8 轮）后按间隔（默认每 6 轮）异步增量抽取，跨会话「越用越懂你」，且评估探针数据绝不污染正式记忆。
...
### 跨会话持久记忆（对标 Claude Code）

普通 RAG 智能体靠「滑动窗口 + 话题摘要」记住对话，但两者都绑定在 `conversationId` 上——**会话结束，记忆即失效**，下次开新会话一切从零。用户第 5 次对话，智能体仍然不知道「这是法务部的张三」、仍然要重新听一遍「预算 5 万以内、要中文回答」。

SparkX 的持久记忆让智能体**跨会话沉淀长期事实**，从「一次性问答工具」进化为「越用越懂你的长期伙伴」。这套机制对标 Claude Code 的 CLAUDE.md 持久记忆 + wU2 结构化分区摘要。

**四层记忆体系**

| 层级 | 机制 | 隔离维度 | 生命周期 | 对标 Claude Code |
|---|---|---|---|---|
| L1 滑动窗口 | 最近 N 轮原样消息 | conversationId | 单会话 | verbatim tail |
| L2 话题摘要 | 本会话话题索引（只记话题不记答案） | conversationId | 单会话 | 会话内 auto-compact |
| **L3 持久记忆** | 跨会话长期事实（结构化 JSON） | agentId + adminId | 跨会话持久 | CLAUDE.md + wU2 分区 |
| 系统提示词 | 智能体人设 / 角色 | agentId | 永久（人工编辑） | 系统级指令 |

> L1/L2 是会话内记忆，会话结束即不再加载；L3 是跨会话记忆，同一管理员在同一智能体下的**所有会话共享**，与 L1/L2 **并行、互补、不替代**。

**设计亮点**

- **会话族隔离**：记忆 key = `pmem:{agentId}:{adminId}`。不同智能体（法务 / 财务）记忆独立；同一智能体服务不同管理员时，每人有独立的用户画像与偏好，互不串味。
- **结构化分区**：持久记忆是固定四分区 JSON（用户画像 / 长期约束 / 已确认事实 / 交互偏好），而非自由文本——信息密度高、注入简单。**只记「用户是谁、要什么、偏好什么」，不记具体答案**，避免与实时 RAG 检索的最新文档冲突。
- **异步增量抽取**：每轮回答后尝试由 `@Async` 线程触发抽取，但**不是每轮都抽**——四重闸门（全局开关 → 智能体开关 → 起步阈值 8 轮 → 间隔节流每 6 轮）把 LLM 调用压到极低频：用户消息积累满 8 轮才首次抽取，之后自上次抽取起每新增约 6 轮（12 条消息）才再抽一次。复用 `rewriteModel` 小快模型降本，旧记忆增量合并去重，新事实冲突以本次为准。
- **评估数据零污染**：评估链路传入 `null` 的 memoryKey 不触发自身抽取，且增量抽取在 SQL 层显式排除 `conversation_id LIKE '%:eval:%'` 的探针会话——**双重保障**确保评估数据绝不污染正式记忆。
- **注入顺序**：作为独立 `SystemMessage` 插入人设之后、会话历史之前，四层结构共同构成完整上下文。

```
messages = [
  SystemMessage(人设)                     // agentId 固定
  SystemMessage(<persistent-memory>)     // ★ L3 跨会话长期事实
...
> 完整设计（写入路径、增量合并算法、抽取提示词、与三种记忆方案对比）见 `xservice` 文档站《跨会话持久记忆设计》。
