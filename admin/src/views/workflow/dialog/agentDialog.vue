...
<script setup>
  ...
  import { getAgentEnabledList } from '@/api/system/agent';
  ...
  onMounted(async () => {
    try {
      const res = await getAgentEnabledList();
  ...
  function onAgentChange(val) {
    ...
    const a = agentOptions.value.find((x) => x.value === val);
    form.value.agentName = a ? a.label : '';
    emitChange();
  }
  function emitChange() {
