// 节点参数保存/调试前校验。返回 {code, msg}，code != 0 表示校验失败。
export default {
	check(graphData) {
		for (let node of graphData.cells) {
			...
			if (node.shape === 'switch-node') {
				...
				for (let branch of node.data.ifBranch) {
					for (let item of branch.data) {
						// input 为 [{nodeId, field}]（新）或 [nodeId, field]（旧），需至少一项完整
						const hasInput =
							Array.isArray(item.input) &&
							item.input.length > 0 &&
							(item.input[0] instanceof Object
								? item.input[0].nodeId && item.input[0].field
								: item.input[0] && item.input[1]);
						if (!hasInput || item.tips === '' || item.value === '') {
							return { code: -11, msg: '请设置【条件分支' + name + '】的条件', data: [] };
						}
					}
				}
			} else if (node.shape === 'dataset-node') {
				// 知识库检索节点
			...
			} else if (node.shape === 'answer-node') {
				// 回复节点
