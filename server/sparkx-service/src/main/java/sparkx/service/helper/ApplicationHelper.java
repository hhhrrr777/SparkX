...
public class ApplicationHelper {
    ...
    /**
     * 构建调用可见性
     * @return AiServiceListener
     */
    public AiServiceListener observability() {

        return new AiServiceListener() {

            @Override
            public void onEvent(AiServiceEvent event) {
                log.info("=== LangChain4j 可观测性 - AI服务事件 ===");
                log.info("事件类型: {}", event.getClass().getSimpleName());
                log.info("事件详情: {}", event.toString());
                log.info("=== LangChain4j 可观测性 - 事件结束 ===");
            }
